...
// --- PERBAIKAN: Impor SVG sebagai komponen React ---
// Pastikan path ini sesuai dengan struktur folder Anda.
// Jika ikon tidak ada, Anda perlu membuatnya atau mengganti dengan path yang benar.
// Untuk saat ini, saya akan tetap menggunakan path string seperti sebelumnya,
// namun dengan menerapkan properti `unoptimized` pada komponen Image untuk Tools.
// Ikon SVG untuk social media akan saya ganti dengan path placeholder karena tidak ada di file.
// Anda bisa menggantinya dengan komponen SVG atau menggunakan react-icons.
...
const ProjectCard = ({ title, tech, imgSrc, onClick }: { title: string, tech: string[], imgSrc: string | null, onClick: () => void }) => {
    const cardVariants: Variants = { initial: { opacity: 0, y: 20 }, inView: { opacity: 1, y: 0, transition: { duration: 0.5 } } };
    ...
    return (
        ...
        >
            {/* --- PERBAIKAN: Menambahkan unoptimized={true} untuk semua gambar --- */}
            {imgSrc && <Image src={imgSrc} alt={title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="rounded-lg object-cover object-top transition-transform duration-500 ease-in-out group-hover:scale-110" unoptimized={true} />}
    ...
    return (
        <>
            ...
            <main className="px-4 md:px-8">
                ...
                <motion.section id="about" className="py-24 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 items-start" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                                    ...
                                    <div key={tool.id} className="transition-transform hover:scale-110" title={tool.name}>
                                        {tool.icon && (
                                            <div className="relative w-14 h-14">
                                                {/* --- PERBAIKAN: Menambahkan unoptimized={true} untuk SVG dan format lain --- */}
                                                <Image src={tool.icon} alt={tool.name} fill sizes="56px" className="object-contain" unoptimized={true} />
                                            </div>
                                        )}
                                    </div>
                                ))}
