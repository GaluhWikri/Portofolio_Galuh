...
const ProjectCard = ({ title, tech, imgSrc }: { title: string, tech: string[], imgSrc: string }) => {
    ...
    return (
        <motion.div className="relative rounded-lg overflow-hidden group h-80 shadow-lg" variants={cardVariants}>
            <Image src={imgSrc} alt={title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition-transform duration-500 ease-in-out group-hover:scale-110"/>
    ...
    return (
        <>
            ...
            <main className="px-4 md:px-8">
                ...
                {/* About Section */}
                <motion.section id="about" className="py-24 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
                    <motion.div className="md:col-span-2 text-left space-y-8" variants={sectionAnimation} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                        <div>
