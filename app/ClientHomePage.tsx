...
// Dynamic import untuk komponen yang hanya berjalan di client-side
...
const ProjectCard = ({ title, tech, imgSrc }: { title: string, tech: string[], imgSrc: string }) => {
    ...
    return (
        <motion.div className="relative rounded-lg overflow-hidden group h-80 shadow-lg" variants={cardVariants}>
            {/* Pastikan imgSrc ada sebelum render Image */}
            {imgSrc && <Image src={imgSrc} alt={title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition-transform duration-500 ease-in-out group-hover:scale-110"/>}
            <motion.div className="absolute inset-0 bg-black bg-opacity-60" variants={overlayVariants} initial="initial" whileHover="hover" transition={{ duration: 0.3 }}>
            ...
            <main className="px-4 md:px-8">
                    ...
                    <motion.div variants={sectionAnimation} custom={1} className="h-full">
                        <div className="bg-white text-black h-full min-h-[300px] rounded-2xl p-6 flex flex-col items-center">
                            ...
                            <div className="grid grid-cols-3 gap-y-4 gap-x-2 w-full place-items-center">
                                {tools.map((tool: any) => (
                                    <div key={tool.id} className="flex flex-col items-center justify-center p-2 transition-transform hover:scale-110" title={tool.name}>
                                        {tool.icon && <div className="relative w-12 h-12"><Image src={tool.icon} alt={tool.name} fill sizes="48px" className="object-contain" /></div>}
                ...
                <motion.section id="project" className="py-24 max-w-5xl mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {projects.map((project: any) => (<ProjectCard key={project.id} title={project.title} tech={project.tech} imgSrc={project.imgSrc} />))}
                    </div>
                </motion.section>
