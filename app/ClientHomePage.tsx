...
export default function ClientHomePage({ tools }: { tools: { name: string, icon: string }[] }) {
    ...
    // Variabel educationText tidak lagi digunakan, bisa dihapus
    ...
    const cvPath = "/assets/cv/Galuh%20Wikri%20Ramadhan.pdf";
                ...
                >
                    ...
                    >
                        <div>
                            ...
                            <motion.button
                                onClick={() => setIsCvVisible(true)}
                                className="mt-6 px-6 py-2 border border-gray-500 text-white font-semibold rounded-lg transition-all hover:bg-white hover:text-black focus:outline-none focus:ring-2 focus:ring-white"
                        ...
                        {/* ==== BAGIAN EDUCATION DIPERBARUI DI SINI ==== */}
                        <div>
                            <h2 className="text-4xl font-bold mb-4">Education</h2>
                            <div className="flex justify-between items-start font-sans text-lg">
                                <div>
                                    <p className="font-semibold text-white">Universitas Pasundan</p>
                                    <p className="text-gray-400">Informatic Engineering</p>
                                </div>
                                <p className="text-gray-400 text-right">October 2022 - Present</p>
                            </div>
                        </div>
                    ...
                    <motion.div variants={sectionAnimation} custom={1}>
                        <div className="bg-white text-black h-full min-h-[300px] rounded-2xl p-6 flex flex-col items-center">
                            ...
                            <div className="grid grid-cols-3 gap-y-4 gap-x-2 w-full place-items-center">
                                {tools.map(tool => (
                                    <div key={tool.name} className="flex items-center justify-center p-2 rounded-lg transition-transform hover:scale-110" title={tool.name}>
                                        <div className="relative w-12 h-12">
                                            <Image src={tool.icon} alt={`${tool.name} icon`} layout="fill" objectFit="contain" />
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {isCvVisible && (
                    ...
                    >
                        <motion.div
                            ...
                            className="relative w-[90vw] h-[90vh] bg-gray-900 rounded-lg overflow-hidden shadow-2xl"
                            onClick={(e) => e.stopPropagation()}
                        >
                            <object data={cvPath} type="application/pdf" className="w-full h-full">
                                <p className="text-white text-center p-4">
                                    Browser Anda tidak dapat menampilkan PDF. Silakan unduh CV 
                                    <a href={cvPath} download className="text-blue-400 hover:underline"> di sini</a>.
                                </p>
                            </object>
                            <button
                                onClick={() => setIsCvVisible(false)}
                                className="absolute top-4 right-4 text-white bg-black bg-opacity-50 rounded-full w-10 h-10 flex items-center justify-center text-2xl hover:bg-opacity-75 transition-all"
                                aria-label="Tutup"
                            >
                                &times;
                            </button>
