...
// Komponen utama halaman yang dirender di sisi client
export default function ClientHomePage({ data }: { data: any }) {
    const [isCvVisible, setIsCvVisible] = useState(false);
    const [selectedProject, setSelectedProject] = useState<Project | null>(null); // State untuk proyek yang dipilih
    const cvPath = "/assets/cv/Galuh Wikri Ramadhan.pdf";
    const { aboutMe, education, tools, projects } = data;

    const handleProjectClick = (project: Project) => {
        // Fungsi ini memastikan modal hanya terbuka jika ada gambar
        if (project.imgSrc) {
    ...
    return (
        <>
            ...
            {/* Modal untuk menampilkan gambar proyek (Behance Style dengan box transparan) */}
            <AnimatePresence>
                {selectedProject && (
                    <motion.div
                        initial={{ opacity: 0 }}
